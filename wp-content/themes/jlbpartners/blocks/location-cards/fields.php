 ...
 */
function jlbpartners_location_cards_fields() {
	...
	acf_add_local_field_group(
		array(
			'key'    => 'group_jlbpartners_location_cards',
			'title'  => 'Location Cards',
			'fields' => array(
				// Content Tab
				array(
					'key'   => 'field_jlbpartners_location_cards_content_tab',
					'label' => 'Content',
					'type'  => 'tab',
				),
				array(
					'key'         => 'field_jlbpartners_location_cards_title',
					'label'       => 'Title',
					'name'        => 'title',
					'type'        => 'text',
					'required'    => 0,
				...
				array(
					'key'          => 'field_jlbpartners_location_cards_locations',
					'label'        => 'Locations',
					'name'         => 'locations',
					'type'         => 'repeater',
					'layout'       => 'block',
					'collapsed'    => 'field_jlbpartners_location_card_title',
					...
					'sub_fields'  => array(
						array(
							'key'         => 'field_jlbpartners_location_card_title',
							'label'       => 'Title',
							'name'        => 'title',
							'type'        => 'text',
							'required'    => 1,
						...
						array(
							'key'          => 'field_jlbpartners_location_card_content',
							'label'        => 'Content',
							'name'         => 'content',
							'type'         => 'wysiwyg',
							'required'     => 1,
...
add_action( 'acf/init', 'jlbpartners_location_cards_fields' );
 ...
 */
function jlbpartners_location_cards_validate_block_title( $valid, $value, $field, $input_name ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Required by ACF filter signature.
	// Only apply to location cards block title field.
	if ( ! isset( $field['key'] ) || 'field_jlbpartners_location_cards_title' !== $field['key'] ) {
		return $valid;
	}
...
add_filter( 'acf/validate_value', 'jlbpartners_location_cards_validate_block_title', 10, 4 );
 ...
 * @return bool|string True if valid, error message if invalid.
 */
function jlbpartners_location_cards_validate_title( $valid, $value, $field, $input_name ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Required by ACF filter signature.
	// Only apply to location card title field.
	if ( ! isset( $field['key'] ) || 'field_jlbpartners_location_card_title' !== $field['key'] ) {
		return $valid;
	}
...
add_filter( 'acf/validate_value', 'jlbpartners_location_cards_validate_title', 10, 4 );
