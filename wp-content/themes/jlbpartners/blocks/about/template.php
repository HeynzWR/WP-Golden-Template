...
if ( $is_preview && empty( $about_title ) && empty( $about_stats ) && empty( $about_text ) ) {
	jlbpartners_show_block_preview( 'about-section', __( 'About Section Block', 'jlbpartners' ) );
	return;
}
...
if ( $about_image ) {
	...
	$image_width = 800;
	$image_height = 600;
	$image_alt = $about_image_alt ? $about_image_alt : esc_html__( 'Placeholder image', 'jlbpartners' );
}
