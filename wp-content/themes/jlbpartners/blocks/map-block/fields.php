 ...
 */
function jlbpartners_map_block_fields() {
	...
	$fields = array(
		// Content Tab
		array(
			'key'   => 'field_jlbpartners_map_block_content_tab',
			'label' => 'Content',
			'type'  => 'tab',
		),
		// Warning Message
		array(
			'key'           => 'field_jlbpartners_map_block_warning',
			'label'         => '',
			'name'          => '',
			'type'          => 'message',
		...
		array(
			'key'          => 'field_jlbpartners_map_block_location_cards',
			...
			'max'          => 6,
			'layout'       => 'block',
			'collapsed'    => 'field_jlbpartners_map_block_title',
			...
			'sub_fields'   => array(
				array(
					'key'         => 'field_jlbpartners_map_block_title',
					'label'       => 'Title',
					'name'        => 'title',
					'type'        => 'text',
					'required'    => 1,
				...
				array(
					'key'         => 'field_jlbpartners_map_block_name',
					'label'       => 'Name',
					'name'        => 'name',
					'type'        => 'text',
					'required'    => 1,
				...
				array(
					'key'         => 'field_jlbpartners_map_block_subtitle',
					'label'       => 'Subtitle',
					'name'        => 'subtitle',
					'type'        => 'text',
					'required'    => 1,
				...
				array(
					'key'         => 'field_jlbpartners_map_block_content_info_title',
				...
				array(
					'key'          => 'field_jlbpartners_map_block_content_info',
					'label'        => 'Content Info',
					'name'         => 'content_info',
					'type'         => 'wysiwyg',
					'required'     => 1,
				...
				array(
					'key'          => 'field_jlbpartners_map_block_cta',
					'label'        => 'CTA',
					'name'         => 'cta',
					'type'         => 'link',
	...
	acf_add_local_field_group(
		array(
			'key'    => 'group_jlbpartners_map_block',
			'title'  => 'Map Block',
			'fields' => $fields,
			'location' => array(
				array(
...
add_action( 'acf/init', 'jlbpartners_map_block_fields' );
 ...
 */
function jlbpartners_map_block_validate_title( $valid, $value, $field, $input_name ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Required by ACF filter signature.
	// Only apply to map block title field.
	if ( ! isset( $field['key'] ) || 'field_jlbpartners_map_block_title' !== $field['key'] ) {
		return $valid;
	}
	...
	if ( $text_length > 100 ) {
		$valid = sprintf(
			...
			__( 'Title must not exceed 100 characters. Current length: %d characters.', 'jlbpartners' ),
			$text_length
		);
	}
	return $valid;
}
add_filter( 'acf/validate_value', 'jlbpartners_map_block_validate_title', 10, 4 );
 ...
 * @return bool|string True if valid, error message if invalid.
 */
function jlbpartners_map_block_validate_subtitle( $valid, $value, $field, $input_name ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Required by ACF filter signature.
	// Only apply to map block subtitle field.
	if ( ! isset( $field['key'] ) || 'field_jlbpartners_map_block_subtitle' !== $field['key'] ) {
		return $valid;
	}
	...
	if ( $text_length > 100 ) {
		$valid = sprintf(
			/* translators: %d: Maximum character limit */
			__( 'Subtitle must not exceed 100 characters. Current length: %d characters.', 'jlbpartners' ),
			$text_length
		);
	}
	return $valid;
}
add_filter( 'acf/validate_value', 'jlbpartners_map_block_validate_subtitle', 10, 4 );
 ...
 * @return bool|string True if valid, error message if invalid.
 */
function jlbpartners_map_block_validate_name( $valid, $value, $field, $input_name ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Required by ACF filter signature.
	// Only apply to map block name field.
	if ( ! isset( $field['key'] ) || 'field_jlbpartners_map_block_name' !== $field['key'] ) {
		return $valid;
	}
	...
	if ( $text_length > 100 ) {
		$valid = sprintf(
			/* translators: %d: Maximum character limit */
			__( 'Name must not exceed 100 characters. Current length: %d characters.', 'jlbpartners' ),
			$text_length
		);
	}
	return $valid;
}
add_filter( 'acf/validate_value', 'jlbpartners_map_block_validate_name', 10, 4 );
 ...
 * @return bool|string True if valid, error message if invalid.
 */
function jlbpartners_map_block_validate_content_info_title( $valid, $value, $field, $input_name ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Required by ACF filter signature.
	// Only apply to map block content info title field.
	if ( ! isset( $field['key'] ) || 'field_jlbpartners_map_block_content_info_title' !== $field['key'] ) {
		return $valid;
	}
	...
	if ( $text_length > 100 ) {
		$valid = sprintf(
			/* translators: %d: Maximum character limit */
			__( 'Content Info Title must not exceed 100 characters. Current length: %d characters.', 'jlbpartners' ),
			$text_length
		);
	}
	return $valid;
}
add_filter( 'acf/validate_value', 'jlbpartners_map_block_validate_content_info_title', 10, 4 );
