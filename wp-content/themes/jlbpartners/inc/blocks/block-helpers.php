 ...
 * @return void
 */
function jlbpartners_show_block_preview( $block_name, $block_title ) {
	...
	if ( ! empty( $preview_image_path ) ) {
		// Show image preview.
		?>
		<div class="jlbpartners-block-preview" style="position: relative; border: 2px dashed #00a400; border-radius: 8px; overflow: hidden;">
			<img 
			...
			<div style="position: absolute; bottom: 10px; right: 10px; background: rgba(0, 0, 0, 0.7); color: white; padding: 6px 10px; border-radius: 4px; font-size: 11px;">
				<?php esc_html_e( 'Add content to customize', 'jlbpartners' ); ?>
			</div>
		</div>
		<?php
	} else {
		...
		<div class="jlbpartners-block--preview" style="min-height: 300px; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%); border: 2px dashed #00a400; border-radius: 8px; padding: 40px; text-align: center;">
			<div class="jlbpartners-block__preview-placeholder" style="max-width: 500px;">
				...
				<p style="margin: 0 0 8px 0; color: #646970; font-size: 14px; line-height: 1.6;">
					<?php esc_html_e( 'Add content to get started with this block.', 'jlbpartners' ); ?>
				</p>
				<div style="margin-top: 20px; padding: 12px; background: rgba(255, 255, 255, 0.7); border-radius: 4px; font-size: 12px; color: #646970;">
					...
					<?php printf( esc_html__( 'Place %s-preview.jpg in:', 'jlbpartners' ), esc_html( $block_name ) ); ?><br>
 ...
 */
function jlbpartners_sanitize_text( $text ) {
	$allowed_tags = array(
		'strong' => array(),
		'em'     => array(),
		'br'     => array(),
 ...
 */
function jlbpartners_get_optimized_image_attrs( $image_id, $size = 'full', $args = array() ) {
	$defaults = array(
		'lazy'    => true,
		'srcset'  => true,
		'sizes'   => '100vw',
