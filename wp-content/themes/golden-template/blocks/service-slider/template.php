...
if ( $is_preview && empty( $service_items ) ) {
	golden_template_show_block_preview( 'service-slider', __( 'Service Slider Block', 'golden-template' ) );
	return;
}
					...
					<div class="service-block__image-slider js-service-image-slider">
						<?php foreach ( $service_items as $item ) : ?>
							...
							if ( $service_image_id ) {
								...
								$image_width = 800;
								$image_height = 1200;
								$image_alt = $service_title ? $service_title : esc_html__( 'Placeholder image', 'golden-template' );
							}
