 ...
 * @return bool
 */
function golden_template_has_yoast() {
 ...
 * @return string
 */
function golden_template_get_seo_title() {
	if ( golden_template_has_yoast() && function_exists( 'YoastSEO' ) ) {
 ...
 * @return string
 */
function golden_template_get_seo_description() {
	if ( golden_template_has_yoast() && function_exists( 'YoastSEO' ) ) {
 ...
 */
function golden_template_schema_markup() {
	if ( is_front_page() ) {
		$schema_type = 'WebSite';
	} elseif ( is_single() ) {
		$schema_type = 'Article';
 ...
 */
function golden_template_ensure_yoast_og() {
	if ( ! golden_template_has_yoast() ) {
		add_action( 'wp_head', 'golden_template_add_basic_og_tags', 5 );
	}
}
add_action( 'wp', 'golden_template_ensure_yoast_og' );

/**
 * Add basic Open Graph tags if Yoast is not active
 */
function golden_template_add_basic_og_tags() {
	global $post;
	?>
 ...
 */
function golden_template_add_canonical() {
	if ( ! golden_template_has_yoast() && is_singular() ) {
		echo '<link rel="canonical" href="' . esc_url( get_permalink() ) . '">' . "\n";
	}
}
add_action( 'wp_head', 'golden_template_add_canonical', 10 );

/**
 * Add breadcrumb support for Yoast
 */
function golden_template_yoast_breadcrumbs() {
	if ( golden_template_has_yoast() && function_exists( 'yoast_breadcrumb' ) ) {
 ...
 * @return array
 */
function golden_template_get_schema_data() {
	$schema = array(
