...
function jlbpartners_allowed_block_types( $allowed_blocks, $editor_context ) {
	...
	if ( 'page' === $post_type ) {
		...
		// For other pages: restrict to only JLBPartners blocks.
		return array(
			'acf/hero-section',
			'acf/test-hero-block',
			'acf/about-section',
...
add_filter( 'allowed_block_types_all', 'jlbpartners_allowed_block_types', 10, 2 );

/**
 * Customize block editor settings
 */
function jlbpartners_editor_settings( $settings ) {
...
add_filter( 'block_editor_settings_all', 'jlbpartners_editor_settings', 10, 1 );

/**
 * Enqueue custom editor scripts
 */
function jlbpartners_custom_editor_scripts() {
	wp_enqueue_script(
		'jlbpartners-editor-customization',
		get_template_directory_uri() . '/assets/js/admin/editor-customization.js',
		array( 'wp-blocks', 'wp-dom-ready', 'wp-edit-post' ),
		jlbpartners_get_asset_version( 'assets/js/admin/editor-customization.js' ),
		true
	);
	...
	wp_enqueue_style(
		...
		array(),
		jlbpartners_get_asset_version( 'assets/css/admin/editor-customization.css' )
	);
}
add_action( 'enqueue_block_editor_assets', 'jlbpartners_custom_editor_scripts' );

/**
 * Add custom body class to editor
 */
function jlbpartners_editor_body_class( $classes ) {
	$classes .= ' jlbpartners-editor';
	return $classes;
}
add_filter( 'admin_body_class', 'jlbpartners_editor_body_class' );

/**
 * Add editor help text based on post type
 */
function jlbpartners_editor_notices() {
...
add_action( 'admin_footer', 'jlbpartners_editor_notices' );

/**
 * Remove unwanted editor panels based on post type
 */
function jlbpartners_remove_editor_panels() {
	$screen = get_current_screen();
...
add_action( 'admin_footer', 'jlbpartners_remove_editor_panels' );

/**
 * Customize block inserter search
 */
function jlbpartners_block_inserter_help() {
	?>
	<style>
		.edit-post-header-toolbar__inserter-toggle,
		.edit-site-header-edit-mode__inserter-toggle {
			background: var(--jlbpartners-primary, #00a400) !important;
		...
		.edit-site-header-edit-mode__inserter-toggle:hover {
			background: var(--jlbpartners-primary-dark, #004500) !important;
		}
		...
		.block-editor-block-types-list__item[data-id^="acf/"] .block-editor-block-icon {
			color: var(--jlbpartners-primary, #00a400);
		}
...
add_action( 'admin_head', 'jlbpartners_block_inserter_help' );
