...
if ( $is_preview && empty( $hero_title ) ) {
	jlbpartners_show_block_preview( 'hero-section', __( 'Hero Section Block', 'jlbpartners' ) );
	return;
}
...
if ( $background_image ) {
	...
	$bg_image_url = jlbpartners_get_placeholder_image();
	$bg_image_width = 1920;
	$bg_image_height = 1080;
	$bg_image_alt = esc_html__( 'Placeholder image', 'jlbpartners' );
}
								...
								<figcaption class="visually-hidden">
									...
									<?php if ( $background_video_transcript_url ) : ?>
										<a href="<?php echo esc_url( $background_video_transcript_url ); ?>" target="_blank" rel="noopener noreferrer">
											<?php esc_html_e( 'View transcript', 'jlbpartners' ); ?>
										</a>
									<?php endif; ?>
								</figcaption>
							<?php endif; ?>
					...
					<?php else : ?>
						...
						<figure class="hero__figure" aria-hidden="true">
							<img 
								...
								alt="<?php esc_attr_e( 'Placeholder image', 'jlbpartners' ); ?>"
								loading="lazy"
								role="presentation"
							>
						</figure>
