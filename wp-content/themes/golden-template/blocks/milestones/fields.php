 ...
 */
function golden_template_milestones_fields() {
	...
	acf_add_local_field_group(
		array(
			'key'    => 'group_golden_template_milestones',
			'title'  => 'Milestones',
			'fields' => array(
				array(
					'key'          => 'field_golden_template_milestones_items',
					...
					'sub_fields'   => array(
						...
						array(
							'key'          => 'field_golden_template_milestone_title',
...
add_action( 'acf/init', 'golden_template_milestones_fields' );
 ...
 */
function golden_template_milestones_validate_label( $valid, $value, $field, $input_name ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- Required by ACF filter signature.
	// Only apply to milestone label field.
	if ( isset( $field['key'] ) && 'field_golden_template_milestone_label' === $field['key'] ) {
		// Count characters.
		$text_length = mb_strlen( $value );
		if ( $text_length > 4 ) {
			$valid = sprintf(
				/* translators: %d: Maximum character limit */
				__( 'Label / Year must not exceed 4 characters. Current length: %d characters.', 'golden-template' ),
				$text_length
			);
		}
	}
...
add_filter( 'acf/validate_value', 'golden_template_milestones_validate_label', 10, 4 );
