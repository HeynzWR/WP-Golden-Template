...
if ( $is_preview && empty( $leadership_cards ) ) {
	jlbpartners_show_block_preview( 'filter-cards', __( 'Filter Cards Block', 'jlbpartners' ) );
	return;
}
...
$default_active_tag = '';
?>
<section id="<?php echo esc_attr( $block_id ); ?>" class="<?php echo esc_attr( $class_name ); ?>" role="region" aria-label="<?php echo esc_attr__( 'Filter Cards', 'jlbpartners' ); ?>">
	<div class="container">
		<div class="jlb filter-blocks">
			...
			<?php if ( ! empty( $active_tags ) ) : ?>
				<ul class="filter-blocks__nav" aria-label="<?php echo esc_attr__( 'Filter options', 'jlbpartners' ); ?>" data-aos="fade-up">
					<?php
					$filter_index = 0;
