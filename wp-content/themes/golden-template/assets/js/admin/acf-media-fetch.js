...
(function($) {
	...
	function addAllFetchButtons($scope) {
		...
		if ($fieldWrap.find('.golden_template-fetch-btn').length) {
			return;
		}
	...
	function addFetchButtonToField(targetField, sourceFieldName, metadataKey) {
		...
		if ($targetField.find('.golden_template-fetch-btn').length) {
			return false;
		}
		...
		const $button = $('<button>', {
			type: 'button',
			class: 'golden_template-fetch-btn',
	...
	function showMetadataPreview(targetField, metadataKey, attachmentData) {
		...
		const modalHtml = `
			<div class="golden_template-metadata-modal">
				<div class="golden_template-metadata-modal__overlay"></div>
				<div class="golden_template-metadata-modal__content">
					<div class="golden_template-metadata-modal__header">
						<h3>📥 Media Metadata: ${formatMetadataKey(metadataKey)}</h3>
						<button class="golden_template-metadata-modal__close" aria-label="Close">×</button>
					</div>
					<div class="golden_template-metadata-modal__body">
						<div class="golden_template-metadata-status ${statusClass}">
							<span class="status-icon">${statusIcon}</span>
							<span class="status-text">${statusText}</span>
						</div>
						${!isEmpty ? `
							<div class="golden_template-metadata-preview">
								<label>Media Library ${formatMetadataKey(metadataKey)}:</label>
								<div class="metadata-value">${escapeHtml(metadataValue)}</div>
							</div>
						` : `
							<div class="golden_template-metadata-empty">
					...
					<div class="golden_template-metadata-modal__footer">
						${!isEmpty ? `
							<button class="button button-primary golden_template-metadata-apply" data-value="${escapeHtml(metadataValue)}">
								✅ Use This Data
							</button>
						` : ''}
						<button class="button golden_template-metadata-cancel">Cancel</button>
					</div>
				</div>
			</div>
		`;
		...
		$('.golden_template-metadata-apply').on('click', function() {
			const value = $(this).data('value');
			applyMetadata(targetField, value);
			closeModal();
		});

		$('.golden_template-metadata-cancel, .golden_template-metadata-modal__close, .golden_template-metadata-modal__overlay').on('click', closeModal);
	}
	 ...
	 */
	function closeModal() {
		$('.golden_template-metadata-modal').fadeOut(200, function() {
			$(this).remove();
		});
	}
