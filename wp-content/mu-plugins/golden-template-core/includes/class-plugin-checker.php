				...
				<?php foreach ( $missing as $plugin ) : ?>
					<li>
						...
						<?php if ( $plugin['installed'] ) : ?>
							- <span style="color: #d63638;"><?php esc_html_e( 'Installed but not activated', 'golden-template-core' ); ?></span>
							<?php if ( current_user_can( 'activate_plugins' ) ) : ?>
								- <a href="<?php echo esc_url( admin_url( 'plugins.php' ) ); ?>"><?php esc_html_e( 'Activate Now', 'golden-template-core' ); ?></a>
							<?php endif; ?>
						<?php else : ?>
							- <span style="color: #d63638;"><?php esc_html_e( 'Not installed', 'golden-template-core' ); ?></span>
							<?php if ( $plugin['is_premium'] ) : ?>
								- <?php esc_html_e( 'This is a premium plugin. Please install manually.', 'golden-template-core' ); ?>
							<?php else : ?>
								<?php if ( current_user_can( 'install_plugins' ) ) : ?>
									- <a href="<?php echo esc_url( admin_url( 'plugin-install.php?s=' . rawurlencode( $plugin['slug'] ) . '&tab=search&type=term' ) ); ?>"><?php esc_html_e( 'Install Now', 'golden-template-core' ); ?></a>
								<?php endif; ?>
							<?php endif; ?>
						<?php endif; ?>
