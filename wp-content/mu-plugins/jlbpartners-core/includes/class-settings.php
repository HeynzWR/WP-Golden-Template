 ...
 */
class JLBPartners_Core_Settings {
	 ...
	 * @var string
	 */
	private $page_slug = 'jlbpartners-settings';
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_jlbpartners_upload_logo', array( $this, 'handle_logo_upload' ) );
		add_action( 'wp_ajax_jlbpartners_upload_placeholder', array( $this, 'handle_placeholder_upload' ) );
	...
	public function add_settings_page() {
		add_menu_page(
			__( 'JLB Partners', 'jlbpartners-core' ),
			__( 'JLB Partners', 'jlbpartners-core' ),
			'manage_options',
			$this->page_slug,
		...
		add_submenu_page(
			...
			__( 'Project Listing Settings', 'jlbpartners-core' ),
			__( 'Listing Settings', 'jlbpartners-core' ),
			'manage_options',
			'projects',
	...
	public function register_settings() {
		...
		// Footer settings.
		register_setting( 'jlbpartners_footer', 'jlbpartners_footer_address', array( 'type' => 'string', 'sanitize_callback' => 'wp_kses_post', 'default' => '' ) );

		// Project listing settings.
		register_setting(
			'jlbpartners_projects',
			'jlbpartners_projects_page_title',
			array(
				'type'              => 'string',
		...
		// Update settings.
		register_setting( 'jlbpartners_updates', 'jlbpartners_update_api_endpoint' );
	}
	...
	public function enqueue_admin_assets( $hook ) {
		...
		wp_enqueue_script(
			'jlbpartners-admin',
			JLBPARTNERS_CORE_URL . 'assets/js/admin.js',
			array( 'jquery', 'media-upload', 'media-views', 'media-editor' ),
			JLBPARTNERS_CORE_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			array(
				...
				'nonce'   => wp_create_nonce( 'jlbpartners_admin_nonce' ),
			)
		);
		...
		wp_add_inline_script(
			'jlbpartners-admin',
			'const JLBPARTNERS_CORE_URL = "' . esc_js( JLBPARTNERS_CORE_URL ) . '";',
			'before'
		);
		...
		wp_enqueue_style(
			'jlbpartners-admin',
			JLBPARTNERS_CORE_URL . 'assets/css/admin.css',
			array(),
			JLBPARTNERS_CORE_VERSION
		);
		
		// Add inline styles as fallback.
		$inline_css = file_get_contents( JLBPARTNERS_CORE_PATH . 'assets/css/admin.css' );
		...
		";
		wp_add_inline_script( 'jlbpartners-admin', $inline_js );
	}
	...
	public function render_settings_page() {
		...
		$logo_id         = get_option( 'jlbpartners_logo', 0 );
		$logo_url        = $logo_id ? wp_get_attachment_url( $logo_id ) : '';
		$placeholder_id  = get_option( 'jlbpartners_placeholder_image', 0 );
		$placeholder_url = $placeholder_id ? wp_get_attachment_url( $placeholder_id ) : '';
		
		// Update settings.
		$api_endpoint = get_option( 'jlbpartners_update_api_endpoint', '' );

		// Get system info.
		$update_manager  = jlbpartners_core()->update_manager;
		$branding        = jlbpartners_core()->branding;
		$plugin_checker  = jlbpartners_core()->plugin_checker;
		...
		require JLBPARTNERS_CORE_PATH . 'templates/settings-page.php';
	}
	...
	public function handle_logo_upload() {
		check_ajax_referer( 'jlbpartners_admin_nonce', 'nonce' );

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'Insufficient permissions', 'jlbpartners-core' ) ) );
		}

		$attachment_id = isset( $_POST['attachment_id'] ) ? absint( $_POST['attachment_id'] ) : 0;

		if ( $attachment_id ) {
			update_option( 'jlbpartners_logo', $attachment_id );
			wp_send_json_success( array( 'message' => __( 'Logo updated', 'jlbpartners-core' ) ) );
		}

		wp_send_json_error( array( 'message' => __( 'Invalid attachment ID', 'jlbpartners-core' ) ) );
	}
	...
	public function handle_placeholder_upload() {
		...
		if ( $attachment_id ) {
			update_option( 'jlbpartners_placeholder_image', $attachment_id );
			wp_send_json_success( array( 'message' => __( 'Placeholder image updated', 'jlbpartners-core' ) ) );
		}
	...
	public function render_projects_settings_page() {
		...
		require JLBPARTNERS_CORE_PATH . 'templates/projects-settings-page.php';
	}
