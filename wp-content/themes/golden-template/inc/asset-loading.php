 ...
 */
function golden_template_get_used_blocks() {
	global $post;
 ...
 */
function golden_template_extract_block_names( $blocks ) {
	$block_names = array();
	...
	foreach ( $blocks as $block ) {
		...
		if ( ! empty( $block['innerBlocks'] ) ) {
			$inner_block_names = golden_template_extract_block_names( $block['innerBlocks'] );
 ...
 */
function golden_template_enqueue_aos_js() {
	if ( ! wp_script_is( 'aos', 'enqueued' ) ) {
		// Use theme URI constant if available, otherwise fallback to WordPress function
		$theme_uri = defined( 'GOLDEN_TEMPLATE_THEME_URI' ) ? GOLDEN_TEMPLATE_THEME_URI : get_template_directory_uri();
 ...
 */
function golden_template_enqueue_slick_css() {
 ...
 */
function golden_template_enqueue_slick_js() {
 ...
 */
function golden_template_enqueue_slick_assets() {
	golden_template_enqueue_slick_css();
	golden_template_enqueue_slick_js();
}

/**
 * Enqueue Slick Lightbox CSS
 */
function golden_template_enqueue_slick_lightbox_css() {
 ...
 */
function golden_template_enqueue_slick_lightbox_js() {
 ...
 */
function golden_template_enqueue_slick_lightbox_assets() {
	golden_template_enqueue_slick_lightbox_css();
	golden_template_enqueue_slick_lightbox_js();
}

/**
 * Add inline critical CSS for above-the-fold content
 */
function golden_template_inline_critical_css() {
	...
	if ( ! empty( $critical_css ) ) {
		...
		echo '<style id="golden-template-critical-css">' . esc_html( $critical_css ) . '</style>' . "\n";
	}
}
 ...
 */
add_action( 'wp_enqueue_scripts', 'golden_template_enqueue_aos_js', 5 );
