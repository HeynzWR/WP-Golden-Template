 ...
 */
function jlbpartners_get_post_data_for_autofill() {
	check_ajax_referer( 'jlbpartners_autofill_nonce', 'nonce' );
...
add_action( 'wp_ajax_jlbpartners_get_post_data', 'jlbpartners_get_post_data_for_autofill' );

/**
 * AJAX handler to get featured image for a post
 */
function jlbpartners_get_featured_image_for_autofill() {
	check_ajax_referer( 'jlbpartners_autofill_nonce', 'nonce' );
...
add_action( 'wp_ajax_jlbpartners_get_featured_image', 'jlbpartners_get_featured_image_for_autofill' );

/**
 * AJAX handler to filter projects
 */
function jlbpartners_filter_projects() {
	...
	// Query projects
	$projects_per_page = absint( get_option( 'jlbpartners_projects_per_page', 2 ) );
	...
	$selected_filters_html = jlbpartners_get_filter_tags_html( $selected_states, $selected_types );
	...
	// Generate pagination
	$pagination_html = jlbpartners_generate_pagination_html( $projects_query, $page, $selected_states, $selected_types );
...
add_action( 'wp_ajax_filter_projects', 'jlbpartners_filter_projects' );
add_action( 'wp_ajax_nopriv_filter_projects', 'jlbpartners_filter_projects' );
 ...
 */
function jlbpartners_get_filter_display_text( $selected_states, $selected_types ) {
 ...
 */
function jlbpartners_get_filter_tags_html( $selected_states, $selected_types ) {
	$html = '';
 ...
 */
function jlbpartners_generate_pagination_html( $query, $page, $selected_states, $selected_types ) {
 ...
 */
function jlbpartners_get_available_filters() {
	check_ajax_referer( 'jlbpartners_nonce', 'nonce' );
	...
	if ( empty( $selected_states ) && empty( $selected_types ) ) {
		...
		$available_states = jlbpartners_get_available_terms_optimized( 'project_state', $all_states, $selected_types, 'project_type' );
		$available_types  = jlbpartners_get_available_terms_optimized( 'project_type', $all_types, $selected_states, 'project_state' );
	}
...
add_action( 'wp_ajax_get_available_filters', 'jlbpartners_get_available_filters' );
add_action( 'wp_ajax_nopriv_get_available_filters', 'jlbpartners_get_available_filters' );
 ...
 */
function jlbpartners_get_available_terms_optimized( $check_taxonomy, $all_terms, $selected_other, $other_taxonomy ) {
	global $wpdb;
	...
	$cached_slugs    = wp_cache_get( $cache_key_query, 'jlbpartners' );
	...
	if ( false !== $available_slugs ) {
		wp_cache_set( $cache_key_query, $available_slugs, 'jlbpartners', 5 * MINUTE_IN_SECONDS );
	}
 ...
 */
function jlbpartners_clear_filter_cache( $post_id ) {
	...
	// Clear object cache
	wp_cache_flush_group( 'jlbpartners' );
}
add_action( 'save_post', 'jlbpartners_clear_filter_cache' );
add_action( 'delete_post', 'jlbpartners_clear_filter_cache' );
add_action( 'wp_trash_post', 'jlbpartners_clear_filter_cache' );
add_action( 'untrash_post', 'jlbpartners_clear_filter_cache' );
